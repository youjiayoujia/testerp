...
console.log(console.log((function ($, win, undef) {
		...
		check: function (curform, brothers, subpost, bool) {
			...
			if (!flag.passed) {
				...
				if (!bool) {
					...
					Validform.util.showmsg.call($this, flag.info, settings.tiptype, {
						obj: $(this),
						type: flag.type,
				...
				if ($(this).attr("ajaxurl") && !bool && $(this).attr("ignore") != "ignore") {
					...
					Validform.util.showmsg.call($this, brothers.tipmsg.c || tipmsg.c, settings.tiptype, {
						obj: inputobj,
						type: 1,
						sweep: settings.tipSweep
					}, "hide");
					...
					_this[0].validform_ajax = $.ajax({
						...
						success: function (s) {
							if ($.trim(s) == "y") {
								...
								Validform.util.showmsg.call($this, brothers.tipmsg.r || tipmsg.r, settings.tiptype, {
									obj: inputobj,
									type: 2,
									sweep: settings.tipSweep
								}, "hide");
								...
								_this.addClass("Validform_error");
								Validform.util.showmsg.call($this, s, settings.tiptype, {
									obj: inputobj,
									type: 3,
									sweep: settings.tipSweep
								});
						...
						error: function (data) {
							if (data.statusText !== "abort") {
								_this.addClass("Validform_error");
								Validform.util.showmsg.call($this, brothers.tipmsg.err || tipmsg.err, settings.tiptype, {
									obj: inputobj,
									type: 3,
									sweep: settings.tipSweep
								});
				...
				if (settings.ajaxPost || ajaxPost === "ajaxPost") {
					...
					curform[0].validform_ajax = $.ajax({
						...
						success: function (data) {
							...
							if (data.status === "copyright") {
								if (confirm('有侵权的词汇,如果要忽略侵权词继续发布，请确认后，点击再次发布!否则点击取消!')) {
									proform.attributes['action'].value = proform.attributes['starurl'].value + '?copysure=yes';
								} else {
									proform.attributes['action'].value = proform.attributes['starurl'].value;
								}
							}
							if (data.status === "y") {
...
//	$(".registerform").Validform();  //就这一行代码！
	var index = $(".registerform").attr("ajaxpost");
