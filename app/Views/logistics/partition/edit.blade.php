@extends('common.form')
@section('formAction') {{ route('logisticsPartition.update', ['id' => $model->id]) }} @stop
@section('formBody')
   ...
   <input type='hidden' name='hideUrl' value="{{$hideUrl}}">
    <div class='row'>
        <div class="form-group col-lg-6">
            ...
            <input type='text' class="form-control" id="name" placeholder="物流分区名" name='name' value="{{ old('name') ? old('name') : $model->name }}">
        </div>
    </div>
    <div class='row'>
        <div class='form-group col-lg-6'>
            ...
            <select class='form-control country_id' name='country_id[]' multiple>
                ...
                @foreach($countries as $country)
                    <option value="{{ $country->id }}" {{ $country->id ? in_array($country->id, $arr) ? 'selected' : '' : ''}}>{{$country->cn_name}}</option>
                @endforeach
            </select>
        </div>
    </div>
