@extends('common.form')
@section('formAction') {{ route('overseaAllotment.update', ['id' => $model->id]) }} @stop
@section('formBody')
    ...
    <div class='row'>
        <div class="form-group col-lg-3">
            ...
            @foreach($outWarehouses as $warehouse)
                <option value='{{ $warehouse->id }}' {{old('out_warehouse_id') ? (old('out_warehouse_id') == $warehouse->id ? 'selected' : '') : ($model->out_warehouse_id == $warehouse->id ? 'selected' : '')}}>{{ $warehouse->name }}</option>
            @endforeach
            </select>
        </div>
        <div class="form-group col-lg-3">
