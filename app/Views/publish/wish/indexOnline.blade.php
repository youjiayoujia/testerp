...
@section('tableHeader')
    ...
    <th class="sort" data-field="number_saves">收藏量</th>
    <th>创建人员</th>
    <th>刊登时间</th>
    <th>操作</th>
@stop
@section('tableBody')
    @foreach($data as $wishProduct)
        <tr>
            ...
            <td>{{  $wishProduct->number_saves }}</td>
            <td>
            ...
            <td>
                ...
                @if($wishProduct->product_type_status==2)
                    ...
                    <a onclick="operator('<?php echo $wishProduct->id;  ?>' ,'disable',this)"
                       class="btn btn-danger btn-xs  <?php   if (!$wishProduct->status == 1) {
                           echo "hidden";
                       }      ?>">
                        <span class="glyphicon glyphicon-pencil "></span> 下架
                    </a>

                    <a onclick="operator('<?php echo $wishProduct->id;  ?>' ,'enable',this)"
                       class="btn btn-success btn-xs <?php   if ($wishProduct->status == 1) {
                           echo "hidden";
                       }      ?>">
