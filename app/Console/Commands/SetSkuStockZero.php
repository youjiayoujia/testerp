...
class SetSkuStockZero extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'autoSetSkuStockZero:setSkuZero';
     ...
     * @var string
     */
    protected $description = '系统自动下调(货源待定)产品的可售库存';
    ...
    {
        $skuList = ItemModel::where(['status'=>'unSellTemp','is_available'=>1])->get();     //获取所有货源待定状态的sku      
        foreach($skuList as $sku){           
           $virtualStock = $sku->available_quantity;
           $zaitu = $sku->normal_transit_quantity; 
           if($virtualStock + $zaitu <= 1){ //获取 虚库存+在途 <= 1的sku               
